...
public class 전국대회선발고사 {
	...
	public static void main(String[] args) {
		 ...
		 * 3명을 선발해야 하지만, 개인 사정으로 전국 대회에 참여하지 못하는 학생들이 있어 참여가 가능한 
		 * 학생 중 등수가 높은 3명을 선발하기로 했습니다.각 학생들의 선발 고사 등수를 담은 정수 배열 
		 * rank와 전국 대회 참여 가능 여부가 담긴 boolean 배열 attendance가 매개변수로 주어집니다. 
        ...
        for (int i = 0; i < list.size() - 1; i++) {
            for (int j = i + 1; j < list.size(); j++) {
                if (rank[list.get(i)] > rank[list.get(j)]) {
                    int tmp = list.get(i);
